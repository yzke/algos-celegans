...
def neural_step(V, c, sensory_input, W_chem, W_gap, S):
    ...
    return V_new
参数:
- `tanh(β*V)`,β = 1。tanh 的输出在 [-1, 1],V=0 处输出 0,使
  V=0 在无驱动时是真正的不动点。tanh 与 logistic 的关系为
  `tanh(x) = 2*σ(2x) - 1`,因此 `tanh(βV)` 与 `σ(2βV) - 0.5` 完全等价
  (相差一个 2× 振幅因子)。
...
> 饱和。Phase 0 用了 `σ(V) - 0.5` 作为临时修正(等价于 `0.5 * tanh(βV/2)`);
> v0.3 正式把激活函数改写为 `tanh(β*V)`,这是同一类的中心化非线性,无需
> magic 常数,且与 Beer/Izquierdo 一系 CTRNN 文献的写法一致。
>
> β 的取值变化: 直接的字面替换 `tanh(5V)` 比 Phase 0 的等价形式
> `0.5*tanh(2.5V)` 陡两倍且无 0.5 振幅压制,会把 V 钉在 ±1 附近,
> 网络功能上崩塌。Phase 0.5 通过 β 扫描确定 β = 1 在我们这套
> per-row-L1 归一化的 W_chem 上是临界值: β ≤ 1 时 V=0 是唯一稳定不动点,
> β ≳ 1.05 时网络出现非零吸引子。选 β=1 既消除了 Phase 0 "无输入仍有
> max|V| ≈ 0.35 残余活动" 的伪影,又让 phase0.md §3.2 原始的
> `max|V| < 0.1` 严格判据重新成立。
